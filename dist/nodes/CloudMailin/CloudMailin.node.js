...
class CloudMailin {
    constructor() {
        this.description = {
            ...
            description: 'Manage CloudMailin inbound addresses, custom domains, sending domains, ' +
        ...
        for (let i = 0; i < items.length; i++) {
            try {
                ...
                if (resource === 'inboundAddress') {
                    if (operation === 'getAll') {
                        responseData = await GenericFunctions_1.cloudMailinApiRequest.call(this, 'GET', '/addresses');
                    }
                    if (operation === 'get') {
                        const addressId = this.getNodeParameter('addressId', i);
                        responseData = await GenericFunctions_1.cloudMailinApiRequest.call(this, 'GET', `/addresses/${addressId}`);
                    }
                    if (operation === 'create') {
                        ...
                        responseData = await GenericFunctions_1.cloudMailinApiRequest.call(this, 'POST', '/addresses', body);
                    }
                    if (operation === 'update') {
                        const addressId = this.getNodeParameter('addressId', i);
                        ...
                        if (updateFields.targetFormat)
                            body.target_format = updateFields.targetFormat;
                        if (updateFields.nickname)
                            body.nickname = updateFields.nickname;
                        responseData = await GenericFunctions_1.cloudMailinApiRequest.call(this, 'PATCH', `/addresses/${addressId}`, body);
                    }
                    if (operation === 'delete') {
                        const addressId = this.getNodeParameter('addressId', i);
                        responseData = await GenericFunctions_1.cloudMailinApiRequest.call(this, 'DELETE', `/addresses/${addressId}`);
                    }
                }
                // ── INBOUND MESSAGE ──────────────────────────────────────────────────
                if (resource === 'inboundMessage') {
                    if (operation === 'getAll') {
                        ...
                        responseData = await GenericFunctions_1.cloudMailinApiRequest.call(this, 'GET', endpoint);
                    }
                }
                // ── CUSTOM DOMAIN ────────────────────────────────────────────────────
                if (resource === 'customDomain') {
                    if (operation === 'getAll') {
                        const addressId = this.getNodeParameter('addressId', i);
                        responseData = await GenericFunctions_1.cloudMailinApiRequest.call(this, 'GET', `/custom_domains?address_id=${addressId}`);
                    }
                    if (operation === 'create') {
                        const addressId = this.getNodeParameter('addressId', i);
                        ...
                        const body = { domain };
                        if (additionalFields.authRegex)
                            body.auth_regex = additionalFields.authRegex;
                        if (additionalFields.domainTarget)
                            body.domain_target = additionalFields.domainTarget;
                        responseData = await GenericFunctions_1.cloudMailinApiRequest.call(this, 'POST', `/custom_domains?address_id=${addressId}`, body);
                    }
                    if (operation === 'delete') {
                        const addressId = this.getNodeParameter('addressId', i);
                        const customDomainId = this.getNodeParameter('customDomainId', i);
                        responseData = await GenericFunctions_1.cloudMailinApiRequest.call(this, 'DELETE', `/custom_domains/${customDomainId}?address_id=${addressId}`);
                    }
                }
                // ── EMAIL (SEND) ─────────────────────────────────────────────────────
                if (resource === 'email') {
                    if (operation === 'send') {
                        ...
                        const bodyType = this.getNodeParameter('bodyType', i);
                        const bodyContent = this.getNodeParameter('body', i);
                        const additionalFields = this.getNodeParameter('emailAdditionalFields', i);
                        const emailBody = {
                            from,
                        ...
                        if (additionalFields.priority)
                            emailBody.priority = additionalFields.priority;
                        if (additionalFields.tags) {
                            const tags = additionalFields.tags;
                            emailBody.tags = tags.includes(',') ? tags.split(',').map((t) => t.trim()) : tags;
                        }
                        if (additionalFields.testMode)
                            emailBody.test_mode = additionalFields.testMode;
                        responseData = await GenericFunctions_1.cloudMailinSmtpRequest.call(this, 'POST', '/messages', emailBody);
                    }
                }
                // ── OUTBOUND MESSAGE ─────────────────────────────────────────────────
                if (resource === 'outboundMessage') {
                    if (operation === 'getAll') {
                        const outboundAccountId = this.getNodeParameter('outboundAccountId', i);
                        const query = this.getNodeParameter('query', i);
                    ...
                    if (operation === 'get') {
                        const messageId = this.getNodeParameter('outboundMessageId', i);
                        responseData = await GenericFunctions_1.cloudMailinApiRequest.call(this, 'GET', `/outbound_messages/${messageId}`);
                    }
                }
                // ── SENDING DOMAIN ───────────────────────────────────────────────────
                if (resource === 'sendingDomain') {
                    ...
                    if (operation === 'get') {
                        const sendingDomainId = this.getNodeParameter('sendingDomainId', i);
                        responseData = await GenericFunctions_1.cloudMailinApiRequest.call(this, 'GET', `/sending_domains/${sendingDomainId}`);
                    }
                    if (operation === 'create') {
                        const domain = this.getNodeParameter('sendingDomainName', i);
                        responseData = await GenericFunctions_1.cloudMailinApiRequest.call(this, 'POST', '/sending_domains', {
                            domain,
                        });
                    }
                    if (operation === 'delete') {
                        const sendingDomainId = this.getNodeParameter('sendingDomainId', i);
                        responseData = await GenericFunctions_1.cloudMailinApiRequest.call(this, 'DELETE', `/sending_domains/${sendingDomainId}`);
                    }
                    if (operation === 'verify') {
                        const sendingDomainId = this.getNodeParameter('sendingDomainId', i);
                        responseData = await GenericFunctions_1.cloudMailinApiRequest.call(this, 'POST', `/sending_domains/${sendingDomainId}/verify`);
                    }
                }
                // Push response data
...
exports.CloudMailin = CloudMailin;
